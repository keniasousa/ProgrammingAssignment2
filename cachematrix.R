...
makeCacheMatrix <- function(x = matrix()) {
  ...
  getsolve <- function() s
  #return a list with a function to set and get the value of  matrix, 
...
cacheSolve <- function(x, ...) {
        ...
        #data gets the matrix passed as parameter when makeCacheMatrix is called.
        data <- x$get()
        # s gets the inverse of the matrix.
        s <- solve(data,...)
        #the inverse of the matrix is saved in the variable s in cache.
        x$setsolve(s)
        x
}
...
#cached gets the return of cacheSolve function, which is the list of four functions
